...
class EmbeddedClassTree():
    ...
    def embed_classes(self, classes=None):
        ...
        classes = [cl.split(' ') if isinstance(cl, str) else
                   cl for cl in classes]
        self.class_vectors = np.array(
            [self.embedding.embed_multi_words(words) for words in classes])
    ...
    def normalize_class_tree(self, tree):
        ...
        '''
        tree = {name: rels for (name, rels) in tree.items()
                if self.embedding.in_vocab(name)}
        classes = list(tree.keys())  # filtered class list

        # remove filtered classes from parent and child lists
        for cl, rels in tree.items():
            tree[cl]['children'] = [
                child for child in rels['children'] if (child in classes)]
            tree[cl]['parents'] = [
                parent for parent in rels['parents'] if (parent in classes)]
...
def tree_score(score_map, tree, agg_func):
    ...
    def process_layer(layer):
        assert(layer)
        for node in layer:
            agg_score[node] = apply_agg_func(
                node, score_map, tree, agg_score, agg_func)
...
def to_class_name(onto_class_obj,
                  replace_chars=[DASHES_TO_SPACES, REMOVE_PAREN]):
...
def generate_class_tree_file(ontology_path='ontologies/dbpedia_2016-10.nt',
                             prune=False):
    ...
    if prune:
        ...
        relationships = {name: rels for (name, rels) in relationships.items()
                         if has_relations(rels)}

    print('number of ontology classes:', len(ont.classes))

    print('number of all classes (including children & parents):',
          len(all_classes))

    print('number of relationships keys:', len(relationships.keys()))

    print('classes minus rel keys:', 
          all_classes.difference(set(relationships.keys())))

    print('writing class relationships to file')
    ontology_name = path_to_name(ontology_path)
    file_name = 'ontologies/class-tree_{0}{1}.json'.format(
        ontology_name, '_pruned' if prune else '')
